    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 16 }}>
        {[
          ...
          { label: 'Pendiente de cobro', value: fmt(pendGlobal), color: pendGlobal > 0 ? '#b07a1e' : '#333' },
        ].map((k) => (
          ...
          {pendPorCliente.length === 0 ? (
            ...
            pendPorCliente.map((c) => (
              <div key={c.nombre} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '7px 0', borderBottom: '1px solid #f4f2ed', fontSize: 12.5, gap: 12 }}>
                ...
                <span style={{ fontWeight: 600, color: '#b07a1e', whiteSpace: 'nowrap', fontVariantNumeric: 'tabular-nums' }}>{fmt(c.pend)}</span>
              </div>
            ))
          )}
        </div>
